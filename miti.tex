...
as a \cc{malloc} size, programmers need a non-trivial check \cc{(nsize
\&\& n > SIZE_MAX / size)}.  When the expression gets more complex,
such as \cc{a * b + c * d}, programmers would have to write checks for
...
We modify \cc{malloc} to take an \cc{nan size_t} as the
allocation size and return \cc{NULL} if the size is NaN.
...
Below is an example.
\input{code/use-nan}
In the multiplication \cc{c * sizeof(int)}, the type of \cc{n} is
\cc{nan size_t}, so \cc{sizeof(int)} is promoted to \cc{nan size_t}.
If the multiplication overflows, the value of \cc{bytes} becomes NaN.

We also change \cc{malloc} to take an \cc{nan size_t} as the
allocation size and return \cc{NULL} if the size is NaN.
