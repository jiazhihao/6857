...
\end{itemize}
Our modified compiler inserts error checks for every arithmetic or
...
%
The compiler then generates efficient overflow
detection instructions depending on the architecture.  For example,
on x86 the compiler may use the \cc{jno} instruction, which jumps
if no overflow happened.
