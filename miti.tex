...
integer overflows, to detect integer errors at run time.

The main concern of enabling trapping semantics in the comiler for
production code is that it introduces false alarms at run time,
while there is no convenient way to specify fine-grained error
handlers.  It also incurrs noticeable runtime overhead~\cite{ioc},
given the prevalence of integer operations.
