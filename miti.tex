...
\zj{Should we change \cc{unnan} API to R\ \cc{unnan}(\cc{nan}\ T\; x,\; R\; v), 
therefore programmers can write codes like \cc{if (unnan(x < y, true))}, where \cc{x}, \cc{y} are NaN Type}

\if 0
\zj{above doesn't explain the return value of comparison with NaN Type, necessary to say?} 
\fi
...
use \cc{unnan} with \cc{if} statements, and tells the compilers what to do
when the result is NaN.

We modify \cc{malloc} to take an \cc{nan size_t} as
...
as the result.
%
In w/o \cc{malloc} cases, the manual check incurs $700\%$ slowdown, while
...
check via NaN integers, measured in cycles, for both w/o and w/ \cc{malloc} cases.}
