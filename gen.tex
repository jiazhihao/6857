...
OOB and path predicates are generated on a per-function basis, and do not
capture constraints on external values, such as function parameters and memory
loads. To mitigate false positives, \sys generates range predicates that impose
global constraints on these external value sources.
...
Linux kernel. Instead, \sys bounds each external value using two constant
integers as the minimum and maximum values, respectively. For example, if the
range of a parameter $x$ is from 1 to 10, \sys generates the range predicates
$x \geq 1 \land x < 11$.

\sys implements an interprocedural value range propagation algorithm to infer
the ranges of all integer-typed function parameters, return values, structure
members and global variables. The algorithm maintains a global range table that
maps each of the aforementioned external values to their ranges. Initially, all
ranges are set to an empty set, except for values that originate from untrusted
external sources, such as system call arguments, which are initialized to 
full sets.

The range table is consulted each time when a function accesses its arguments,
uses values returned by a function call, loads from a structure member or
global variables.  Within the function, value ranges are propagated using range
arithmetics~\cite{range-analysis}.  When a value reaches an
external sink through argument passing, function returns, stores to structure
members or global variables, the corresponding range table entry is updated by
merging its previous range with the range of the incoming value. The algorithm
iteratively scans through every function of the entire system, update the
range table accordingly until it converges. If some table entries do not
converge within a limited number of rounds, they are assumed to be full sets.

The above algorithm requires a system-wide call graph that associates each call
site with a set of possible callee functions. \sys builds the graph beforehand
using a similar iterative algorithm. For each indirect call site, \sys collects
possible assignments to the function pointer by looking at all initializers and
stores to the corresponding structure member, variable or parameter. 

To further reduce false errors, we also refine the output ranges using
domain-specific knowledge. For example, many \cc{sysctl} parameters in the
Linux kernel have lower and upper bounds encoded in their \cc{sysctl} table
entries, which are hard to infer using the above algorithm. \sys extracts the
bounds from the code and patches them to the global range table.
...
To avoid division in $N /_u x > y$, \sys rewrites the check using
Boolector's multiplication overflow detection predicate if $N$ is
a special constant $\uintmax(n)$ (see \autoref{s:gen:oob}).
...
%
See \autoref{s:eval:perf} for an evulation of the speed-up.
