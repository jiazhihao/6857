...
\input{code/alias}
There are four occurrences of \cc{arg->count}, each meaning a load
from memory.  \sys must merge these load instructions, otherwise
\sys would consider the \cc{arg->count} in \cc{copy_from_user}
as a fresh load and the value is unconstrained, despite the fact
that earlier code limits \cc{arg->count} to 128; this would cause
\sys to report a false multiplication overflow.

LLVM's load elimination algorithm is too conservative and does not
fit our goal.  For the above example, it would conservatively
assume that the call to \cc{kmalloc} \emph{may} modify \cc{arg->count}
(e.g., via some global variable that points to \cc{arg}).

\sys merges load instructions as follows.
It first adopts an aliasing assumption: a pointer passed to a
...
other, so as to easily merge identical ones.
%
