...
Given an integer operation, its error constraint is generated
directly from its definition in \autoref{s:sema:constr}.
%
\paragraph{Path constraint.}
The path constraint of an integer operation is a predicate satisfying
which the operation is reachable from the start of that function.
It is used to filter out integer error cases that are infeasible
along the control flow.
%
\paragraph{Range constraint.}
\sys infers range constraints of function parameters and structure
fields across functions.  It also allows users to provide annotations
as constraints for improving accuracy.

\paraend
Both error and path constraints are generated within a single
function; they encode bit-level information of integer operations
for precise reasoning.  Range constraints are more coarse-grained
for better performance.  This rest of this section describes how
\sys generates each type of these constraints.
