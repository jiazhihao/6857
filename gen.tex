...
\sys derives an error predicate for each integer operation.
It is the negation of the corresponding security requirement,
satisfying which implies an error.

\paragraph{Arithmetic operations.}
Take multiplication for an example.  Given two $n$-bit unsigned integers
$x$ and $y$, the security requirement is that their product cannot overflow.
There are several possible error predicates \sys could generate.
A \naive form is:
\begin{equation*}
...
(see \autoref{s:eval:micro} for a performance comparison).
%
...
solved far more efficiently The detail implementation of the predicate
...
\sys employs a simple memory model: a load instruction may return
an arbitrary value (unless it has a range predicate).
It relies on LLVM's load elimination
...
and achieves good performance.  See \xw{eval}.
