...
One may implement the error constraints of integer operations defined
in \autoref{s:sema:constr} as Boolean circuits.  For example, given
...
circuits for implementing these error constraints.  As for
multiplication, Boolector's overflow detection circuit does not
even need to compute the $2n$-bit product first and can be solved
more efficiently~\cite[\chapterautorefname~3.5]{brummayer:phd}.
\sys reuses them to generate error constraints for better performance.


[[[array index]]]

[[[annotations on \cc{copy_from_user}]]]
