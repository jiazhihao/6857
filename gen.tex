...
solver answers ``satisfiable'' with an assignment of variables, there is a
...
From the definition in \autoref{s:sema:sec}, \sys derives a
...
hurt the solver's performance noticeably (see \autoref{s:eval:micro}).
...
As defined \autoref{s:sema:sec}, \sys needs to deal with conversions
...
using two constant integers as the minimum and maximum values,
respectively, for better performance.  Otherwise, if \sys simply
generates a path constraint along the call graph, the size of the
path constraint would grow exponentially, making it difficult to
solve.
...
parameter $x$ is from 1 to 10, \sys adds the range constraints $x \geq
