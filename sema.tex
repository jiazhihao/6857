...
code, though one may find them by invoking GCC with \cc{-Wconversion}
...
depends on the context.  In the muliplication example above,
...
\cc{kmalloc} limit is raised in the future.  \sys considers these
...
A constraint solver implements these error constraints of integer
operations as Boolean circuits for deciding satisfiability.  For
example, a multiplication error detector for two $n$-bit unsigned
integers could be implemented using a $2n$-bit binary multiplier,
which flags an overflow error if any of the most significant $n$
bits of the product is 1.

The Boolector constraint solver~\cite{boolector} constructs more
efficient circuits internally.  For example, its multiplication
error detector does not even need to compute the product first to
detect overflow~\cite[\chapterautorefname~3.5]{brummayer:phd}.  \sys
...
example, $\cc{maxnum}\times_u 16$ may be erroneous if \cc{maxnum}
is large, while $\cc{maxnum} \shl 4$ is considered always secure
as long as \cc{maxinum} is wider than 4 bits.  The guidelines assume
