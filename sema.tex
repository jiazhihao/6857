...
constraint of an integer operation is a Boolean predicate if satisifed
...
the size in a call to \cc{copy_from_user}.
\end{itemize}
...
later used in the call to \cc{copy_from_user} as the size parameter,
...
if \cc{maxnum} is large, while $\cc{maxnum} \shl 4$ is considered
always secure as long as \cc{maxinum} is wider than 4 bits.  \sys
...
smaller chunk of memory), or the \cc{kmalloc} limit is raised in
