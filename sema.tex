...
For example, in \autoref{f:vmscan-sign} the intent of the code was
to reset the value of \cc{total_scan} if the addition overflows and
its value goes negative.  However, the type of \cc{total_scan} is
unsigned, and thus the comparison $\cc{total_scan} <_u 0$ always
...
sanity checks:
according C conversion rules, \cc{optlen} is interpreted as positive
...
the \cc{copy_from_user} call, which interprets the negative value
as a large positive integer and causes a stack overflow on architectures
that do not prevent a large \cc{copy_from_user} size.}
\label{f:ax25-sign}
\end{figure}
