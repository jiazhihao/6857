...
\fk{Isn't it better to have two separate statements: i = x + y, and a[i]? to
...
x +_u y <_u x.
\end{equation*}
This idiom is useful when the sum is needed later in the code.\fk{but this check
  is dangerous so kint replaces it with an equivalent but safe check?}
