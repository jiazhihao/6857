...
because the expected arithmetic product $2^{32}\times
...
For example, in \autoref{f:vmscan-sign} the intent was to reset the
value of \cc{total_scan} if the addition overflows and its value
goes negative.  However, the type of \cc{total_scan} is unsigned,
and thus the comparison $\cc{total_scan} <_u 0$ always evaluates
to false and disables the error handling.

\begin{figure}
\centering
\input{code/vmscan-sign}
\vspace{-1em}
\caption{An integer error in \cc{mm/vmscan.c} of the Linux kernel.
Since \cc{total_scan} is declared as \cc{unsigned long}, the overflow
test $(\cc{total_scan} < 0)$ will always be false, which breaks the
error handling.}
\label{f:vmscan-sign}
\end{figure}

\paragraph{$(2)$ Non-negative value.}
A variable that is used as a data size (as specified using annotations,
which we describe later) or an array index should be non-negative.
...
\nz{Why is \cc{optlen} treated differently in the two if statements?
Explain C rules.}
