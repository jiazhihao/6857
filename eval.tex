...
An interesting non-trival bug is due to specification misread in
...
The vulnerabilies that allow out-out-bounds writes (i.e., buffer
overflow) would break the integrity of the kernel and potentially
allow prviledge escalation.  They can be exploited via local access,
...
the details here.  See \autoref{s:eval:common} for a summary of
...
define bounds (often small integers).
