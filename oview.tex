...
to 256, shown as follows (\cc{maxnum} is numbered for clarification
purpose).
\input{code/bridge-p}
This time the error condition is:
\begin{equation*}
\cc{maxnum}_1 >_u (2^{32} - 1) / 16.
\end{equation*}
The corresponding path condition is:
\begin{align*}
& ((\cc{maxnum}_0 > 256) \land (\cc{maxnum}_1 = 256)) \\
\lor
& (\neg (\cc{maxnum}_0 > 256) \land (\cc{maxnum}_1 = \cc{maxnum}_0)).
\end{align*}
