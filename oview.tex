...
\autoref{f:flow} shows the workflow of a naive \sys implementation.
\sys first performs symbolic analysis to compute the error constraint
and the corresponding path constraint for each integer operation.
...
path constraint is simply true.

\begin{figure}
\centering
\input{figs/flow}
\caption{A simple workflow of how \sys analyzes integer errors.}
\label{f:flow}
\end{figure}
...
purpose).
\input{code/bridge-p}
This time the error constraint remains unchanged.
\begin{equation*}
