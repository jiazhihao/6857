...
errors.  For example, one could strictly enforce that a signed
integer being converted to unsigned must be non-negative.  However,
sign conversions are so commonly seen in C code that doing so would
cause too many false errors~\cite{moy:prefix}.
Developers even count on integer overflows such as $x
+ y < x$ for the correctness of their programs.  \sys needs
