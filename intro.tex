...
errors.  For example, one could classify any signed-to-unsigned
integer conversion from a possibly negative value as an
error~\cite{brumley:rich, moy:prefix, seacord:secure-c}.  However,
such sign conversions are so widely used in C code that doing so
would cause a flood of false errors~\cite{moy:prefix}.  Developers
even count on integer overflows such as $x + y < x$ for the correctness
of their programs.  \sys needs an integer semantics that recognizes
commonly used code patterns while still effectively catches integer
errors.
...
the lighttpd web server, and OpenSSH, the patches of which were accepted
by the developers.  It also reveals that even two previously known
CVE cases were not fixed correctly.
%
...
mitigation strategies.  \autoref{s:relwk} relates \sys to previous
