...
kernel driver.  The size for allocating \cc{rects} is computed
from the multiplication of \cc{arg->num_outputs}, a variable
controlled by userspace, and \cc{sizeof(struct drm_vmw_rect)},
the constant $16$.  If an attacker supplies a big integer, say
\cc{0xf0000000}, for the variable, the multiplication would
wrap, resulting in \cc{0xf0000000 * 16 = 0} for the allocation
size.  Then \cc{rects} is a zero-length array, and the access
\cc{rects[i]} is out of bounds, which may cause a kernel
panic.
