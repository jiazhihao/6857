...
To avoid integer errors developers need to, for example, perform
...
including XXX erroneous patches that the developers have commited
...
Compilers such GCC and Clang have provided compilation options like
\cc{-ftrapv} and \cc{-fcatch-undefined-behavior} to generate runtime
checks for common integer errors.  However, detecting integer errors
at run time is difficult since they may manifest themselves with
abnormal input, which are not seen by typical users but can be
exploited by adversaries.  Besides, a module may only be loaded
with some specific hardware or configuration, making it harder to
eliminate integer errors in that module in testing environments.
